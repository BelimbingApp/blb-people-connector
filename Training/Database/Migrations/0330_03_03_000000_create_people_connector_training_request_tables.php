...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('people_connector_training_requests', function (Blueprint $table): void {
            ...
            $table->unsignedBigInteger('requester_employee_entity_id');
            $table->unsignedBigInteger('department_entity_id')->nullable();
            $table->unsignedBigInteger('course_id')->nullable();
            $table->string('skill_reference', 160)->nullable();
            $table->string('development_action_reference', 160)->nullable();
            $table->unsignedBigInteger('proposed_budget_minor')->nullable();
            $table->char('currency', 3)->nullable();
            $table->string('status', 24);
            $table->unsignedBigInteger('created_by_user_id');
            $table->timestamps();
            $table->index('tenant_id', 'pct_request_tenant_idx');
            $table->unique(['id', 'tenant_id', 'company_entity_id'], 'pct_request_owner_uq');
            $table->unique(['tenant_id', 'request_key'], 'pct_request_key_uq');
            $table->index(['tenant_id', 'company_entity_id', 'status'], 'pct_request_queue_idx');
            ...
            $this->entityReference($table, 'company_entity_id', 'pct_request_company_fk');
            $this->entityReference($table, 'requester_employee_entity_id', 'pct_request_requester_fk');
            $this->entityReference($table, 'department_entity_id', 'pct_request_department_fk');
        });
        ...
        $this->guards();
        $this->registerTable('people_connector_training_requests');
        $this->registerTable('people_connector_training_request_decisions');
    }

    public function down(): void
    {
        $this->dropGuards();
        $this->unregisterTable('people_connector_training_request_decisions');
        $this->unregisterTable('people_connector_training_requests');
        Schema::dropIfExists('people_connector_training_request_decisions');
        Schema::dropIfExists('people_connector_training_requests');
    ...
    {
        if (DB::connection()->getDriverName() === 'pgsql') {
            DB::unprepared('DROP TRIGGER IF EXISTS pct_training_request_decision_immutable ON people_connector_training_request_decisions; DROP FUNCTION IF EXISTS pct_training_request_decision_immutable()');
        } elseif (DB::connection()->getDriverName() === 'sqlite') {
            DB::unprepared('DROP TRIGGER IF EXISTS pct_training_request_decision_update_guard; DROP TRIGGER IF EXISTS pct_training_request_decision_delete_guard');
        }
    }
};
