...
The platform sender uses an owner-managed secret named
`PEOPLE_CONNECTOR_DISPATCH_TOKEN`. Prefer a GitHub App installation token; a
fine-grained personal access token is also supported. In either case, select
only `BelimbingApp/blb-people-connector` and grant only repository
`Contents: write`, which GitHub requires for the repository-dispatch endpoint.
Do not reuse a human's general-purpose merge token. A missing or rejected
credential must fail the platform sender visibly rather than silently skipping
the drift check.
