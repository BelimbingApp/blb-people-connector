...
with the failure text on the row). The provider's payload is never stored:
a callback is a trigger, not a second projection-write path.
...
connection and both delivery ids. The audit summary carries no exception
text and no payload.
...
status, attempts, last error, tenant, connection, job and queue) and neither
