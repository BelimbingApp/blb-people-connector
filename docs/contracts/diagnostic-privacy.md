...
| ExportsSupplementalSubjectRecords → subject package `supplemental` block | Module-owned rows about one subject, keyed by table | Called only after `people-connector.identity.export` is authorized, with the subject's tenant and owning company entity; the module redacts by its own rules and the connector's credential/hash redaction runs over the rows again. The audit row carries exporter names and a partial flag, never rows. WorkforceSubjectSupplementalExportTest |
